<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=26&duration=2500&pause=800&color=F72585&center=true&vCenter=true&width=700&lines=SYSTEM+BOOT...+OK;LOADING+BLAZE_O8.EXE;GAME+DEV+%7C+AI+ARCHITECT;REALITY+IS+JUST+ANOTHER+ENGINE" alt="Typing SVG" />

<br>

![Profile banner](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=BLAZE_O8&fontSize=70&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=building%20worlds%20one%20commit%20at%20a%20time&descAlignY=58&descSize=18)

</div>

<div align="center">

`[ SYSTEM ]` I don't just build software — I build **worlds you can walk into**
and **agents that talk back**. Somewhere between game dev and mad scientist,
running on caffeine and questionable sleep schedules.

<img src="https://user-images.githubusercontent.com/74038190/212284158-e840e285-664b-44d7-b79b-e264b5e54825.gif" width="400">

</div>

<br>

## `>_` BOOT SEQUENCE

```diff
+ class        : Game Developer × AI Architect
+ location     : Delhi, India
+ current_arc  : building things that shouldn't exist yet
! weapon_of_choice : JavaScript, Python, Canvas API
! status       : online, caffeinated, shipping
```

<br>

## `>_` WORLD BUILDS

<table>
<tr>
<td width="50%" valign="top">

### 🏙️ pixel-city-game
> A full top-down pixel-art world, hand-built with vanilla JS + Canvas.
> No engine. No shortcuts. Just raw game loop, collision, and a city
> that breathes.

`JavaScript` `Canvas API` `Game Loop` `Zero Dependencies`

</td>
<td width="50%" valign="top">

### 🤖 B.L.A.Z.E — AI Agent
> A personal AI running on Llama 3.3 70B via Groq. Voice control,
> system control, a full web UI — an assistant that actually *does*
> things instead of just talking.

`Python` `Groq` `Llama 3.3` `Voice UI`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🗣️ NL → SQL Query Generator
> Plain English in, working SQL out. Because typing `SELECT * FROM`
> by hand is a grind nobody signed up for.

`Python` `NLP` `SQL`

</td>
<td width="50%" valign="top">

### 🌐 Portfolio
> The site that ties it all together — every build, every skill,
> zero fluff.

`JavaScript` `Design` `Live`

[**→ blazeo8.github.io**](https://blazeo8.github.io/websitetoportfolio/)

</td>
</tr>
</table>

<br>

## `>_` LOADOUT

<div align="center">

![JavaScript](https://img.shields.io/badge/JavaScript-000000?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=3776AB)
![HTML5](https://img.shields.io/badge/HTML5-000000?style=for-the-badge&logo=html5&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-000000?style=for-the-badge&logo=css3&logoColor=1572B6)
![Canvas](https://img.shields.io/badge/Canvas%20API-000000?style=for-the-badge&logo=html5&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-000000?style=for-the-badge&logo=node.js&logoColor=339933)
![Jupyter](https://img.shields.io/badge/Jupyter-000000?style=for-the-badge&logo=jupyter&logoColor=F37626)
![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git&logoColor=F05032)

</div>

<br>

## `>_` LIVE TELEMETRY

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=BlazeO8&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D0D0D&title_color=F72585&icon_color=4CC9F0&text_color=e6e6e6" width="49%" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=BlazeO8&theme=tokyonight&hide_border=true&background=0D0D0D&ring=F72585&fire=4CC9F0&currStreakLabel=e6e6e6" width="49%" />

<img src="https://github-readme-activity-graph.vercel.app/graph?username=BlazeO8&theme=tokyo-night&hide_border=true&bg_color=0D0D0D&color=F72585&line=4CC9F0&point=ffffff" width="99%" />

</div>

<br>

## `>_` CONNECT

<div align="center">

[![Portfolio](https://img.shields.io/badge/Portfolio-F72585?style=for-the-badge&logo=vercel&logoColor=white)](https://blazeo8.github.io/websitetoportfolio/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kartik-jindal-xos53709)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/hehe_kartik_8/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kartikjindals08@gmail.com)

</div>

<div align="center">

```
> connection established. thanks for stopping by.
```

![Profile Views](https://komarev.com/ghpvc/?username=BlazeO8&color=F72585&style=for-the-badge&label=VISITORS)

<br>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./dist/f1-track-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="./dist/f1-track-light.svg">
  <img alt="F1 car racing across contribution graph" src="./dist/f1-track-dark.svg">
</picture>

</div>
